import type { Offset, ZoomMode } from '@pdfrx/viewer';
...
export interface PdfZoom {
  ...
  zoom: number;
  /** Active explicit, fit-page, or fit-width zoom mode. */
  zoomMode: ZoomMode;
  ...
  /** Switches between an explicit zoom, fit-page, and fit-width mode. */
...
export function usePdfZoom(): PdfZoom {
  ...
  const setZoomMode = useCallback(
    (mode: ZoomMode, duration?: number) => store.viewer?.setZoomMode(mode, duration),
    [store],
  );
  ...
  return useMemo(
    () => ({ ...state, zoomIn, zoomOut, setZoom, setZoomMode, fitToPage, fitToWidth, fitToHeight }),
    [state, zoomIn, zoomOut, setZoom, setZoomMode, fitToPage, fitToWidth, fitToHeight],
  );
}
