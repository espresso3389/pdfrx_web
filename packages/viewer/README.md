...
<pdfrx-viewer
  ...
  wasm-modules-url="https://cdn.jsdelivr.net/npm/@pdfrx/engine@0.13.0/assets/"
...
const viewer = new PdfrxViewer(document.getElementById('container')!, {
  engineOptions: {
    wasmModulesUrl: 'https://cdn.jsdelivr.net/npm/@pdfrx/engine@0.13.0/assets/',
  },
});
...
- [`options.interactiveAnnotations`](https://espresso3389.github.io/pdfrx_web/interfaces/_pdfrx_viewer.PdfrxViewerOptions.html#interactiveannotations) — SVG annotation display/editing (on by default); choose drawing or object-selection modes with [`setAnnotationTool()`](https://espresso3389.github.io/pdfrx_web/classes/_pdfrx_viewer.PdfrxViewer.html#setannotationtool) / [`setAnnotationSelectMode()`](https://espresso3389.github.io/pdfrx_web/classes/_pdfrx_viewer.PdfrxViewer.html#setannotationselectmode), configure stroke, fill, opacity, thickness, text color, and font size with [`setAnnotationStyle()`](https://espresso3389.github.io/pdfrx_web/classes/_pdfrx_viewer.PdfrxViewer.html#setannotationstyle), and use [`undoAnnotation()`](https://espresso3389.github.io/pdfrx_web/classes/_pdfrx_viewer.PdfrxViewer.html#undoannotation) / [`redoAnnotation()`](https://espresso3389.github.io/pdfrx_web/classes/_pdfrx_viewer.PdfrxViewer.html#redoannotation) for the shared edit history. `Ctrl`/`Cmd` adds or toggles object selections; a marquee updates selection while it is dragged. Straight lines and arrows use distance-to-segment hit testing (6 screen px for mouse/pen, 10 px for touch), and marquee selection tests their segments instead of their bounding rectangles. Body and anchor drags snap each movable axis to nearby annotation coordinates and show alignment guides; hold `Shift` while dragging a bounding-box or group anchor to preserve its original aspect ratio
...
- Interaction config & callbacks: primary-button background drag and
  right-button drag both pan the document (a moved right drag suppresses the
  browser context menu); two-finger midpoint movement pans while finger
