...
/ `setFormFieldValues()`, and a `formFieldsChanged` event; `@pdfrx/react` exposes the `useFormFields`
...
Each single or bulk API write is one transaction: its event contains all
before/after field changes, including dependent calculation results. An attached
viewer records that complete transaction as one Undo/Redo step.
...
picks *Markup* or *Add link* from the right-click context menu. React's Markup
split action opens a subtype-by-color matrix and remembers the last committed
choice per viewer. Hover/focus calls `previewTextMarkupSelection()` to repaint
the selection non-destructively; closing or leaving the candidate calls
`clearTextMarkupSelectionPreview()`, while clicking commits through
`addTextMarkupToSelection()`. Highlight previews use translucent pastel fills;
the three line subtypes use opaque, higher-contrast strokes. The Canvas preview
and committed SVG overlay share the squiggle-point generator so the preview and
stored annotation use the same wave geometry.
...
created). `highlightSelection()` remains the built-in viewer's Highlight
shorthand; `canAddTextMarkupToSelection()` is the common capability check
...
**Undo/redo** is unlimited and is consumed by the viewer as a stack of command
...
complete reversible `historyChanges`, and `pagesRearranged` includes before/after
arrangements. `formFieldsChanged` likewise includes the complete direct and
calculated field diff for a form transaction. Consequently, direct local
`PdfDocument`/`PdfPage` editing with
the default `origin: 'api'` participates in an attached viewer's history.
`remote`, `restore`, and `history` origins remain observable but are
intentionally not added to local Undo/Redo; page materialization is similarly
excluded through its page-only `materialize` origin.
